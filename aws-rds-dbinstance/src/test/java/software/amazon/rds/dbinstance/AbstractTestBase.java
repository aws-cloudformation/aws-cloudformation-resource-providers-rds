...
public abstract class AbstractTestBase {
    ...
    protected List<DBInstance> computeAssociatedRoleTransitions(DBInstance dbInstance, List<DBInstanceRole> initialRoles, List<DBInstanceRole> finalRoles) {
    ...
    protected Consumer<ProgressEvent<ResourceModel, CallbackContext>> expectInProgress(int pause) {
        return (response) -> {
            assertThat(response).isNotNull();
            assertThat(response.getStatus()).isEqualTo(OperationStatus.IN_PROGRESS);
            assertThat(response.getCallbackDelaySeconds()).isEqualTo(pause);
    ...
    protected Consumer<ProgressEvent<ResourceModel, CallbackContext>> expectSuccess() {
        return (response) -> {
            assertThat(response).isNotNull();
    ...
    protected Consumer<ProgressEvent<ResourceModel, CallbackContext>> expectFailed(final HandlerErrorCode errorCode) {
        return (response) -> {
            assertThat(response).isNotNull();
