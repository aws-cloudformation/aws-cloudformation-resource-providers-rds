...
public class ParameterGrouper implements AbstractGrouper<String, Parameter> {
    private void fillPartitionWithIndependantParameters(Map<String, Parameter> parametersToUpdate, List<Parameter> group, int groupSize) {
        for (String key : parametersToUpdate.keySet()) {
            if (group.size() == groupSize) {
                return;
            }
            group.add(parametersToUpdate.remove(key));
        }
    }

    private void addNewPartitionWithDependantParameters(List<List<Parameter>> partitions, List<List<Parameter>> dependantParameters, int groupSize) {
        ...
        do {
            ...
            for (int i = 0; i < dependantParameters.size(); i++) {
                ...
                if (newPartition.size() + parametersToPartition.size() <= groupSize) {
    ...
    public List<List<Parameter>> partition(Map<String, Parameter> parametersToUpdate, int partitionSize, List<String[]> dependantKeyGroups) {
        ...
        while (numberOfParameterToPartition > 0) {
            ...
            fillPartitionWithIndependantParameters(parametersToUpdate, partitions.get(partitions.size()-1), partitionSize);
