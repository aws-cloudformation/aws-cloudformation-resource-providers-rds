...
public class UpdateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        return ProgressEvent.progress(desiredModel, callbackContext)
                .then(progress -> proxy.initiate("rds::update-cluster-endpoint-precheck", proxyClient, request.getDesiredResourceState(), callbackContext)
                        ...
                        .done((describeRequest, describeResponse, proxyInvocation, model, context) -> {
                            if (!describeResponse.hasDbClusterEndpoints() || describeResponse.dbClusterEndpoints().isEmpty()) {
                                return ProgressEvent.failed(model, context, HandlerErrorCode.NotFound, "DBClusterEndpoint is not found");
                            }
